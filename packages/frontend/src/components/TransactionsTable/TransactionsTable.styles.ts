...
type StyleProps = {
  variant?: TransactionsTableProps['variant']
}
...
export const useStyles = makeStyles<Theme, StyleProps>((theme) => ({
  transactionTable: {
    ...
    borderRadius: ({ variant }) => (variant === 'raised' ? theme.spacing(1) : undefined),
    // filter: ({ variant }) => (variant === 'raised' ? theme.spacing(1) : undefined),
    boxShadow: ({ variant }) => (variant === 'raised' ? theme.shadows[8] : undefined),
    '& thead:first-child th:first-child': {
      borderTopLeftRadius: ({ variant }) => (variant === 'raised' ? theme.spacing(1) : undefined),
    },
    '& thead:first-child th:last-child': {
      borderTopRightRadius: ({ variant }) => (variant === 'raised' ? theme.spacing(1) : undefined),
    },
    '& tfoot tr:last-child, table:not(:has(tfoot)) tbody:last-of-type tr:last-child': {
      '& td:first-child': {
        borderBottomLeftRadius: ({ variant }) =>
          variant === 'raised' ? theme.spacing(1) : undefined,
      },
      '& td:last-child': {
