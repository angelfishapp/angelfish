...
export default function MultiSelectField<T>({
  ...
  getOptionKey = (option: any) =>
    option?.code ?? option.id ?? option.name ?? option.label ?? getOptionLabel(option),
  groupBy,
  ...
  const {
    ...
    myOnChange?.(value); // دالتك التي تأخذ فقط value
  },
    getOptionLabel,
  ...
  const toggleAll = () => {
    if (selectedOptions?.length === options.length) onChange([])
    ...
    <Box sx={{ width: '100%', maxWidth: 500 }}>
      <Paper elevation={3} sx={{ p: 2 }}>
        ...
        >
          ...
          >
            <List disablePadding {...getListboxProps()} formRef={formRef}>
                  ...
                  >
                    <List disablePadding>
                      {group.options.map((option) => (
                        ...
                        >
                          <RenderOption
                            props={{ key: getOptionKey(option) }}
                            option={option as IAccount}
