...
import { Box, Checkbox, Collapse, ListSubheader, Typography } from '@mui/material'
...
export const RenderGroup: React.FC<RenderGroupProps> = ({
  ...
  const handleGroupSelect = (groupName: string, checked: boolean) => {
    if (groupName === 'Account Transfer') {
      const groupOptions = sortedAccounts?.filter((o: IAccount) => o?.class === 'ACCOUNT')
      if (checked) {
  ...
  const isSubGroupChecked = (groupName: string) => {
    if (groupName === 'Account Transfer') {
      const groupOptions = sortedAccounts?.filter((o: IAccount) => o?.class === 'ACCOUNT')
      const someSelected = groupOptions.some((item) => selected?.some((o) => o.name === item.name))

      const allSelected = groupOptions.every((item) => selected?.some((o) => o.name === item.name))
